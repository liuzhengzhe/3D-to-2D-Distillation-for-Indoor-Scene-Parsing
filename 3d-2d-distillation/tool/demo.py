...
def test(model, image_path, classes, mean, std, base_size, crop_h, crop_w, scales, colors):
    ...
    fd = image_path.split('/')[-3].split('.')[0]
    try:
      os.mkdir(os.path.join('./figure/demo/', fd))
    except:
      pass
    gray_path = os.path.join('./figure/demo/', fd+'/'+image_name + '_gray.png')
    color_path = os.path.join('./figure/demo/', fd+'/'+image_name + '_color.png')
