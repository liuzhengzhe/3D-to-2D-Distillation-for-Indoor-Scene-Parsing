...
class Compose(object):
    ...
    def __call__(self, image, label, featidx):
        for t in self.segtransform:
            image, label, featidx = t(image, label, featidx)
        return image, label, featidx
...
class ToTensor(object):
    ...
    def __call__(self, image, label, featidx):
        ...
        featidx = torch.from_numpy(featidx)
        if not isinstance(label, torch.LongTensor):
            label = label.long()
        if not isinstance(featidx, torch.LongTensor):
...
class Resize(object):
    ...
    def __call__(self, image, label, featidx):
        ...
        featidx = cv2.resize(featidx, self.size[::-1], featidx=cv2.INTER_NEAREST)
        return image, label, featidx
...
class RandScale(object):
    ...
    def __call__(self, image, label, featidx):
        ...
        featidx = cv2.resize(featidx, None, fx=scale_factor_x, fy=scale_factor_y, interpolation=cv2.INTER_NEAREST)
        return image, label, featidx
...
class Crop(object):
    ...
    def __call__(self, image, label, feat_idx):
        h, w = label.shape
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            feat_idx = cv2.copyMakeBorder(feat_idx, pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half, cv2.BORDER_CONSTANT, value=0)
        h, w = label.shape
        ...
        feat_idx = feat_idx[h_off:h_off+self.crop_h, w_off:w_off+self.crop_w]
        return image, label, feat_idx
...
class RandRotate(object):
    ...
    def __call__(self, image, label, feat_idx):
        if random.random() < self.p:
            ...
            feat_idx = cv2.warpAffine(feat_idx, matrix, (w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=0)
        return image, label, feat_idx
