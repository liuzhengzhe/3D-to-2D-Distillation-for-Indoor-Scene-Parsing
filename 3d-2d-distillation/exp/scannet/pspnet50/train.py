...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.arch == 'psp':
        ...
        modules_new = [model.ppm, model.reg, model.reg2, model.cls2, model.cls3, model.reg3]
    elif args.arch == 'psa':
        ...
        model = PSANet(layers=args.layers, classes=args.classes, zoom_factor=args.zoom_factor, psa_type=args.psa_type,
                       ...
                       BatchNorm=BatchNorm)
        modules_ori = [model.layer0, model.layer1, model.layer2, model.layer3, model.layer4]
        modules_new = [model.psa, model.cls2, model.aux]
    params_list = []
    ...
    #params_list.append(dict(params=model.bnbias, lr=args.base_lr * 10))
    args.index_split = 5
    ...
    if args.weight:
        if os.path.isfile(args.weight):
            ...
            checkpoint = torch.load(args.weight, map_location=lambda storage, loc: storage.cuda()) #torch.load(args.weight)
            model.load_state_dict(checkpoint,strict=False)#['state_dict'])
            if main_process():
    ...
    if args.evaluate:
        val_transform = transform.Compose([
            ...
            transform.ToTensor(),
            transform.Normalize(mean=mean, std=std)])
        val_data = dataset.SemDataTest(split='val', data_root=args.data_root, data_list=args.val_list, transform=val_transform)
        if args.distributed:
